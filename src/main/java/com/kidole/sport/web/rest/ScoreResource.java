...
/**
 * REST controller for managing {@link com.kidole.sport.domain.Score}.
 */
@RestController
@RequestMapping("/api")
public class ScoreResource {
    ...
    public ResponseEntity<List<ScoreDTO>> searchScores(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Scores for query {}", query);
        Page<ScoreDTO> page = scoreService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
