...
export class ConfrontationResolve implements Resolve<IConfrontation> {
  constructor(private service: ConfrontationService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IConfrontation> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((confrontation: HttpResponse<Confrontation>) => {
          if (confrontation.body) {
            return of(confrontation.body);
          } else {
...
export const confrontationRoute: Routes = [
  {
    ...
    data: {
      ...
      pageTitle: 'kidoleApp.confrontation.home.title'
    },
