...
export class DelegationMembersComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.delegationMembersService
        ...
        .subscribe(
          (res: HttpResponse<IDelegationMembers[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInDelegationMembers(): void {
    this.eventSubscriber = this.eventManager.subscribe('delegationMembersListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IDelegationMembers[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    this.router.navigate(['/delegation-members'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
    ...
    this.delegationMembers = data ? data : [];
  }
