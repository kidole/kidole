...
export class JourneeUpdateComponent implements OnInit {
  ...
  editForm = this.fb.group({
    id: [],
    journeeName: [null, [Validators.required, Validators.minLength(3), Validators.maxLength(1024)]],
    phaseId: []
  });
  ...
  updateForm(journee: IJournee): void {
    this.editForm.patchValue({
      id: journee.id,
      journeeName: journee.journeeName,
  ...
  private createFromForm(): IJournee {
    return {
      ...
      journeeName: this.editForm.get(['journeeName'])!.value,
