...
export class AccreditationComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.accreditationService
        ...
        .subscribe(
          (res: HttpResponse<IAccreditation[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInAccreditations(): void {
    this.eventSubscriber = this.eventManager.subscribe('accreditationListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAccreditation[] | null, headers: HttpHeaders, page: number): void {
